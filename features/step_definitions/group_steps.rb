...
When(/^as an admin I try to edit the name to "(.*?)"$/) do |new_group_name|
  ...
  put "/users/#{@user_id}/groups/#{@group_id}?api_token=#{@api_token}", :group => {:name => new_group_name}    
end

Then(/^the title should be changed to "(.*?)"$/) do |new_group_name|
  visit("/users/#{@user_id}/groups/#{@group_id}?api_token=#{@api_token}")
...
When(/^as an admin I try to edit the date to "(.*?)"$/)  do | new_expiration|
  @user_id= User.find_by_email("dj.vita.09@gmail.com").id  
  put "/users/#{@user_id}/groups/#{@group_id}?api_token=#{@api_token}", :group => {:expiration => new_expiration} 
end
