Given(/^I belong to the group "(.*?)" with members:$/) do |group, members|
  ...
  post "/groups?api_token=#{@api_token}", :group => {:name => group, :expiration => Date.new(2013, 04, 29)}
...
Then(/^the members of the group "(.*?)" should receive the memo with title "(.*?)"$/) do |group, title|
  ...
  @group.users.each do |u|
    visit("/groups/#{@group_id}/memos?api_token=#{@api_token}")
...
When(/^as an admin I try to edit the Title to "(.*?)"$/) do |new_title|
  put "/groups/#{@group_id}/memos/#{@memo_id}?api_token=#{@api_token}", :memo => {:title => new_title}
end

Then(/^the memo Title should be changed to "(.*?)"$/) do |new_title|
  visit("/groups/#{@group_id}/memos/#{@memo_id}?api_token=#{@api_token}")
  page.should have_content(new_title) 
end

#Edit Memo Description
When(/^as an admin I try to edit the Description  to "(.*?)"$/) do |new_description|
  put "/groups/#{@group_id}/memos/#{@memo_id}?api_token=#{@api_token}", :memo => {:description => new_description}
end

#Edit Memo Due Date
When(/^as an admin I try to edit the Due Date  to "(.*?)"$/) do |new_date|
  put "/groups/#{@group_id}/memos/#{@memo_id}?api_token=#{@api_token}", :memo => {:due_date => new_date}
end
